...
and explicit-path mode — including exit codes.
"""
...
class TestScanText(unittest.TestCase):
    ...
    def test_detects_openrouter_key(self):
        findings = secret_scan.scan_text("key=sk-or-v1-abc123\n", "cfg.py")
        self.assertEqual(findings[0][1], "openrouter-key")

    def test_detects_pem_private_key_header(self):
        findings = secret_scan.scan_text("-----BEGIN RSA PRIVATE KEY-----\n", "k.pem")
    ...
    def test_detects_high_entropy_base64_block(self):
        # Two consecutive lines of 40+ base64 chars each.
        b64 = "QUJDREVGR0hJSktMTU5PUFFSU1RVVldYWVphYmNkZWZnaGlqa2xtbm9wcQ=="
        text = f"{b64}\n{b64}\n"
...
class TestCliInGitRepo(TempGitRepoTestCase):
    ...
    def test_tracked_secret_fails(self):
        self._write("leak.py", 't = "ghp_abcdefghij1234567890"\n')
    ...
    def test_staged_mode_ignores_unstaged_secret(self):
        self._write("unstaged_leak.py", 't = "ghp_abcdefghij1234567890"\n')
...
class TestMainInProcess(TempGitRepoTestCase):
    """Drive main() in-process (chdir + argv patch) so the CLI dispatch and
    the git-collection helpers stay visible to coverage — the subprocess
    class above proves the same behavior end-to-end."""
    ...
    def test_staged_mode_fails_on_staged_secret(self):
        self._write("staged_leak.py", 't = "ghp_abcdefghij1234567890"\n')
        self._git("add", "staged_leak.py")
        rc, _out, err = self._run_main("--staged")
        self.assertEqual(rc, 1)
        self.assertIn("github-pat", err)

    def test_git_collection_helpers_return_relative_paths(self):
