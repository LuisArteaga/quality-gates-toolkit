...
jobs:
  quality:
    uses: LuisArteaga/quality-gates-toolkit/.github/workflows/pr-checks.yml@v1.8.2
    with:
      coverage-floor: 80
    secrets:
...
jobs:
  quality:
    uses: LuisArteaga/quality-gates-toolkit/.github/workflows/python-checks.yml@v1.8.2
    with:
      coverage-floor: 80
```
...
llmreview:
  ...
  uses: LuisArteaga/quality-gates-toolkit/.github/workflows/llm-pr-review.yml@v1.8.2
```
...
jobs:
  security:
    uses: LuisArteaga/quality-gates-toolkit/.github/workflows/security.yml@v1.8.2
    with:
      scan-paths: "src"
```
...
| `toolkit-ref` | string | `"v1.8.2"` | Ref of the Python-implementation checkout. Overrides are deliberate. |
...
| `toolkit-ref` | string | `"v1.8.2"` | both — ref of the Python-implementation checkout. Overrides are deliberate. |
...
| `secret-scan.yml` | `toolkit-ref` `"v1.8.2"` | — |
| `diff-coverage.yml` | `toolkit-ref` `"v1.8.2"` · `coverage-artifact` `"coverage-json"` | — |
| `llm-pr-review.yml` | `toolkit-ref` `"v1.8.2"` · `config-path` `"config/factory.json"` · `diff-exclude` `""` · `prefetch-tree-sitter` `false` · `batch-budget-chars` `""` | `openrouter-api-key` (required) · `judge-token` (optional) |
...
jobs:
  js-test:
    uses: LuisArteaga/quality-gates-toolkit/.github/workflows/js-test.yml@v1.8.2
    with:
      node-version: "22"
```
...
```bash
pip install "quality-gates-toolkit @ git+https://github.com/LuisArteaga/quality-gates-toolkit.git@v1.8.2"
```
...
- `uses:` pins an immutable release tag (e.g. `@v1.8.2`); `toolkit-ref`
