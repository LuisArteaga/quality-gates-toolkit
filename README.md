...
jobs:
  quality:
    uses: LuisArteaga/quality-gates-toolkit/.github/workflows/pr-checks.yml@v1.8.6
    with:
      coverage-floor: 80
    secrets:
...
jobs:
  quality:
    uses: LuisArteaga/quality-gates-toolkit/.github/workflows/python-checks.yml@v1.8.6
    with:
      coverage-floor: 80
```
...
llmreview:
  ...
  uses: LuisArteaga/quality-gates-toolkit/.github/workflows/llm-pr-review.yml@v1.8.6
```
...
jobs:
  security:
    uses: LuisArteaga/quality-gates-toolkit/.github/workflows/security.yml@v1.8.6
    with:
      scan-paths: "src"
```
...
| `toolkit-ref` | string | `"v1.8.6"` | Ref of the toolkit checkout — secret scanner, judges, and the Semgrep wrapper `security.yml` runs. Overrides are deliberate. |
...
| `toolkit-ref` | string | `"v1.8.6"` | both — ref of the toolkit checkout — secret scanner, judges, and the Semgrep wrapper `security.yml` runs. Overrides are deliberate. |
...
| `security.yml` | `python-version` `"3.12"` · `scan-paths` `"."` · `enable-semgrep` `true` · `enable-pip-audit` `true` · `toolkit-ref` `"v1.8.6"` | — |
| `secret-scan.yml` | `toolkit-ref` `"v1.8.6"` | — |
| `diff-coverage.yml` | `toolkit-ref` `"v1.8.6"` · `coverage-artifact` `"coverage-json"` | — |
| `llm-pr-review.yml` | `toolkit-ref` `"v1.8.6"` · `config-path` `"config/factory.json"` · `diff-exclude` `""` · `prefetch-tree-sitter` `false` · `batch-budget-chars` `""` | `openrouter-api-key` (required) · `judge-token` (optional) |
...
jobs:
  js-test:
    uses: LuisArteaga/quality-gates-toolkit/.github/workflows/js-test.yml@v1.8.6
    with:
      node-version: "22"
```
...
```bash
pip install "quality-gates-toolkit @ git+https://github.com/LuisArteaga/quality-gates-toolkit.git@v1.8.6"
```
...
- `uses:` pins an immutable release tag (e.g. `@v1.8.6`); `toolkit-ref`
